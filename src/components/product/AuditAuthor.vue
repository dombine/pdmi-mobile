<template lang="html">
  <div class="audit-author" :class="[!headerOptions.visiable?'head-hidden':'']">
    <common-header v-if="headerOptions.visiable" :options="headerOptions"></common-header>
    <div v-if="options.search" class="author-search">
    ...
    <div class="content">
      ...
      <div class="content-body">
        <div v-for="item in data" :key="item.id" @click="onSelect(item)" :class="[initSelect(item.id)?'active':'']">
          <span v-html="redDecorate(item.name)">{{redDecorate(item.name)}}</span>
...
export default {
  ...
  watch: {
    ...
    keyWord: function(val) {
      this.options.refreshCallBack(null, val);
    }
  },
  methods: {
