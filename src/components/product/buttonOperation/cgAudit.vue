<template>
  <div class="cgaudit-operator">
    ...
    <!-- <div class="set-title">编目信息</div>
        <div class="set-container">
          <div class="set-body">
            ...
            <div>
              <input type="text" name="" value="" placeholder="作者" v-model="creator">
            </div>
          </div>
          <div class="set-body" @click="selecttopicPopupVisible = true">
            <img class="img-left" src="../../../assets/image/selecttopic.png" alt="">
            <img class="img-right" src="../../../assets/image/icon/左箭头.svg" alt="">
            <div placeholder="选题">{{selecttopic.name}}</div>
          </div>
          <div class="set-body">
            <img class="img-left" src="../../../assets/image/source.png" alt="">
            <div>
              <input type="text" name="" value="" placeholder="来源" v-model="source">
            </div>
          </div>
          <div class="set-body">
    ...
    <mt-popup class="query-popup" v-model="organizationPopupVisible" position="top">
      <organization :options="organizationOptions()"></organization>
    </mt-popup>

    <mt-popup class="query-popup" v-model="authorPopupVisible" position="top">
      <audit-author :options="authorOptions()"></audit-author>
    </mt-popup>

    <mt-popup class="query-popup" v-model="selecttopicPopupVisible" position="top">
...
import AuditAuthor from '../AuditAuthor.vue'
...
export default {
  ...
  data() {
    return {
      ...
      headerOptions: { title: '提交审核', align: 'center' ,topIndex : true},
  ...
  methods: {
    ...
    commit: function() {
      ...
      let vm = this;
      let url = vm.$global.getUrlByAppId("RESOURCEMANAGER") + '/resources/audit?resourceIdList=' + this.resourceID;
      vm.$http({
        url: url,
        data: JSON.stringify({
          departmentId: vm.organization.id,
          topidId: vm.selecttopic.id,
          auditorId: vm.author.id || vm.organization.id
        }),
        method: 'post',
        headers: { 'Content-Type': 'application/json' }
      })
